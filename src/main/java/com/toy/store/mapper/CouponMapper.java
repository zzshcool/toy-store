...
public interface CouponMapper {
        ...
        @Insert("INSERT INTO coupons (name, code, description, discount_type, discount_value, min_purchase, max_discount, "
                        +
                        "start_date, end_date, usage_limit, used_count, status) " +
                        "VALUES (#{name}, #{code}, #{description}, #{discountType}, #{discountValue}, #{minPurchase}, "
                        +
                        "#{maxDiscount}, #{startDate}, #{endDate}, #{usageLimit}, #{usedCount}, #{status})")
        @Options(useGeneratedKeys = true, keyProperty = "id")
        int insert(Coupon coupon);

        @Update("UPDATE coupons SET name = #{name}, code = #{code}, description = #{description}, " +
