...
public interface IchibanSlotMapper {
    ...
    Optional<IchibanSlot> findByBoxIdAndSlotNumber(@Param("boxId") Long boxId, @Param("slotNumber") Integer slotNumber);
    ...
    @Insert("INSERT INTO ichiban_slots (box_id, slot_number, status, prize_id) " +
            "VALUES (#{box.id}, #{slotNumber}, #{status}, #{prize.id})")
    ...
    @Update("UPDATE ichiban_slots SET status = #{status}, locked_by_member_id = #{lockedByMemberId}, " +
            ...
            "revealed_at = #{revealedAt}, prize_id = #{prize.id} WHERE id = #{id}")
